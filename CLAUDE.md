...
- Older statement imports (`scripts/import_bca_cc.py`, `scripts/reimport_mandiri_jan2026.py`) were hand-transcribed one-off scripts that held real transaction data. They have been removed from the repo and its history. Do not re-add transcribed financial data to the repo; use `parse_bank_statement.mjs` with env credentials instead.
